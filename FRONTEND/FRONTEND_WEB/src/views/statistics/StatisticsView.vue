...
const updateMetrics = () => {
  ...
  metrics.value = [
    { title: '활동 중인 로봇', count: fr.filter(r => r.status !== 'breakdown').length, borderClass: 'border-blue-500' },
    { title: '충전 중인 로봇', count: fr.filter(r => r.status === 'charging').length, borderClass: 'border-green-500' },
    { title: '고장 난 로봇', count: fr.filter(r => r.status === 'broken').length, borderClass: 'border-yellow-500' },
    { title: '수리 중인 로봇', count: fr.filter(r => r.status === 'fixing').length, borderClass: 'border-orange-500' },
  ]
}
