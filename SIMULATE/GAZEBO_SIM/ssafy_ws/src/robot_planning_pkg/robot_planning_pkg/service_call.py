...
home_pose =(304412.92687916575, 3892844.1526765698)
...
class GlobalPathPlanner(Node):
    def __init__(self):
        ...
        # Homing 서비스 서버 생성
        self.srv = self.create_service(Homing, '/test', self.homing_callback)
        ...
        self.get_logger().info("Homing service server ready on /test")
    ...
    def homing_callback(self, request, response):
        """Homing 서비스 요청 처리"""
        #self.goal_pos = (request.goal_x, request.goal_y)
        ...
        if self.current_pos:
            ...
            if path:
                ...
                response.message = "Path calculated and visualized successfully."
            else:
