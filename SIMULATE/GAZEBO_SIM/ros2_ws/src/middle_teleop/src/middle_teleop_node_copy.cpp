...
public:
  ...
  : Node("middle_teleop_node"),
    ...
    up_left_last_time_(0, 0, get_clock()->get_clock_type()),
    up_right_last_time_(0, 0, get_clock()->get_clock_type()),
    down_left_last_time_(0, 0, get_clock()->get_clock_type()),
    down_right_last_time_(0, 0, get_clock()->get_clock_type())
  {
    // key_publisher 토픽 구독
...
private:
  ...
  void keyCallback(const std_msgs::msg::String::SharedPtr msg)
  {
    ...
    if (msg->data == "UP") {
      ...
      up_left_last_time_ = now;
    } else if (msg->data == "UP_RIGHT") {
      up_right_last_time_ = now;
    } else if (msg->data == "DOWN_LEFT") {
      down_left_last_time_ = now;
    } else if (msg->data == "DOWN_RIGHT") {
      down_right_last_time_ = now;
    }
  }
  ...
  void controlLoop()
  {
    ...
    bool up_left = (now - up_left_last_time_).seconds() < 0.2;
    bool up_right = (now - up_right_last_time_).seconds() < 0.2;
    bool down_left = (now - down_left_last_time_).seconds() < 0.2;
    bool down_right = (now - down_right_last_time_).seconds() < 0.2;
  ...
  rclcpp::Time up_left_last_time_, up_right_last_time_, down_left_last_time_, down_right_last_time_;
  ...
  static constexpr double ANGLE_STEP = 0.4; // 동시입력 회전 값 상수, 증가하면 더 빠른 회전 구현 가능
