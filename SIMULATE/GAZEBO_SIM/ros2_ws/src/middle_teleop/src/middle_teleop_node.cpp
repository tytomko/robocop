...
private:
  ...
  void controlLoop()
  {
    ...
    if (space) {
      // 선속도는 BRAKE_FACTOR_LINEAR로, 각속도는 BRAKE_FACTOR_ANGULAR로 각각 감속
  ...
  // 상수들 (필요에 따라 조절하세요)
  static constexpr double ACCEL_STEP       = 0.13;  // 전/후진 시 가속량
  static constexpr double ANGLE_STEP       = 0.07;   // 좌/우 회전 시 가속량
  
  // 예시 변수 설정 (원하는 비율로 조정)
  static constexpr double BRAKE_FACTOR_LINEAR  = 0.04; // 선속도 감속율
  static constexpr double BRAKE_FACTOR_ANGULAR = 0.01; // 각속도 감속율(더 빠르게 감소)

  static constexpr double FRICTION_FACTOR_LINEAR= 0.98; // 선속도 마찰(키 떼면 서서히 감속)
  static constexpr double FRICTION_FACTOR_ANGULAR=0.95; // 각속도 마찰(키 떼면 더 빠르게 or 더 느리게 감속)
